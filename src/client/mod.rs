...
impl ClientState {
    ...
    fn request_repaint(&mut self) {
        ...
        // otherwise a reused content frame would paint a stale sidebar.
...
fn run_client_with_mode(
    ...
    let result = rt.block_on(async {
        ...
        run_client_loop(
            ...
            ClientLoopOptions {
                host_size: (cols, rows),
                draw_host_cursor: attach_escape.is_none()
                    && should_draw_host_cursor(loaded_config.config.ui.host_cursor),
...
fn flush_composited_frame(
    ...
    // Upstream's drawn-cursor path (`ui.host_cursor`), which mx had left as a follow-up: draw the
    // cursor into the frame cells and suppress the host cursor, and honour a pending full repaint
    // so a resize/refresh rewrites every cell without clearing (keeps Kitty graphics intact).
    let frame_data = if state.draw_host_cursor {
        render_ansi::frame_with_drawn_cursor(frame_data)
    } else {
        frame_data
    };
    let repaint = state.repaint_pending;
    let encode_started = Instant::now();
    #[cfg(windows)]
    let encoded = state
        .blit_encoder
        .encode_with_deferred_cursor_reveal(&frame_data, repaint);
    #[cfg(not(windows))]
