...
mod tests {
    ...
    fn stale_release_sequence_is_ignored_for_same_source() {
        ...
        let change = terminal.release_agent_with_mutation("herdr:pi", "pi", Some(19));
    ...
    fn rejected_release_does_not_advance_sequence() {
        ...
        let rejected = terminal.release_agent_with_mutation("herdr:pi", "codex", Some(21));
        assert!(rejected.is_none());
        assert!(terminal.hook_authority.is_some());

        let accepted = terminal.release_agent_with_mutation("herdr:pi", "pi", Some(21));

        assert!(accepted.is_some());
        assert!(terminal.hook_authority.is_none());
        // The anchored session leaves Pi detected, so releasing the hook falls back to the
        // screen-detected state rather than Unknown.
        assert_eq!(terminal.state, AgentState::Idle);
    }
