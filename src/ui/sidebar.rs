...
/// The chevron column of a worktree-group PARENT row. mx's renderer emits the `▸`/`▾` glyph as the
/// first span of the row, so the hit-target is the card's LEFT edge (upstream drew it right-aligned).
...
/// Candidate drop rows for a workspace drag, one per worktree BLOCK (never between a parent and its
/// indented children), plus a trailing slot past the last block. Ported from upstream (#1873) onto
/// mx's `WorkspaceListEntry`, which also carries divider/host-banner entries.
pub(crate) fn workspace_drop_slots(
    ...
    let entry_position = |ws_idx| {
        entries.iter().position(|entry| {
            matches!(
                entry,
                WorkspaceListEntry::Workspace { ws_idx: entry_ws_idx, .. } if *entry_ws_idx == ws_idx
            )
        })
    };
...
/// The screen row the workspace drop indicator draws on for `target`. Keyed off
/// `workspace_drop_slots` so the drawn line always matches the slot the release commits to.
