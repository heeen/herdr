...
mod tests {
    ...
    fn workspace_drop_index_at_row_unaffected_by_divider() {
        ...
        // The result (if any) is a valid insert slot within workspaces bounds.
        if let Some(crate::app::state::WorkspaceDropTarget::Before(insert_idx)) = drop {
    ...
    fn bottom_drop_slot_stays_below_last_workspace_not_footer() {
        ...
        // Only the cards that fit are laid out, so the trailing slot sits before the first
        // workspace that did not fit — not at `End`.
        let bottom_slot = crate::ui::workspace_drop_indicator_row(
            &app.state,
            cards,
            app.state.workspace_list_rect(),
            crate::app::state::WorkspaceDropTarget::Before(cards.len()),
        )
        .unwrap();
