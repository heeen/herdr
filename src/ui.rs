...
use self::mobile::{
    mobile_switcher_max_scroll_for_height, mobile_toast_banner_rect, render_mobile_toast_banner,
};
// The client compositor lays out and renders the mobile header and the switcher panel itself when
// the host is too narrow for a sidebar, so these are shared with the client rather than private to
// the server renderer.
...
pub(crate) use self::{
    ...
    sidebar::{
        ...
        normalized_workspace_scroll, row_host_style, sidebar_section_divider_rect,
        space_sort_toggle_rect, workspace_drop_slots, workspace_group_chevron_rect,
...
pub fn render_with_runtime_registry(
    ...
    // Upstream extracted the sidebar/mobile-header pass into `render_working_animation` so the
    // headless spinner path can repaint it alone; mx inlined the same block here. Call the helper.
    render_working_animation(app, terminal_runtimes, frame);
    render_content_and_overlays(app, terminal_runtimes, frame, app.view.terminal_area, true);
}
